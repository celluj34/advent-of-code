...
";
    ...
    public async Task Execute()
    {
        ...
        var operationPattern = new Regex("^Operation: new = (?<operation>.*)$", RegexOptions.Compiled);
        ...
        var monkeys = Input.Split($"{Environment.NewLine}{Environment.NewLine}", StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
                           .Select(x =>
                           {
                               ...
                               var operation = operationPattern.Match(lines[2]).Groups["operation"].Value;
                               ...
                               return new Monkey(monkeyId, startingItems, $"({operation}) / 3", test, trueActionMonkey, falseActionMonkey);
                           })
                           .ToArray();
        ...
        for (var i = 0; i < 20; i++)
        {
            foreach (var monkey in monkeys)
            {
                ...
                {
                    var result = dt.Compute(monkey.Operation.Replace("old", item.ToString()), "").ToString();
                    var newValue = (int)Math.Truncate(decimal.Parse(result));
    ...
    private record Monkey(int Id, List<int> Items, string Operation, int Test, int TrueActionMonkey, int FalseActionMonkey)
    {
