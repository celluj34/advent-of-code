...
abcccccaaaaaaaaaaaaaaaaaaaaaccccccccccccccccccccccccccccccaaaaaa";
    ...
    public async Task Execute()
    {
        ...
        var bfs = new BreadthFirstPath(Input);
        var rounds = bfs.Search(startX, startY);

        System.Console.WriteLine(rounds);

        //System.Console.WriteLine($"source vertex {source} is has path to target vertex {target} via: {string.Join(" -> ", bfs.PathTo(target))}");
    }
    ...
    private class BreadthFirstPath
    {
        ...
        public BreadthFirstPath(string input)
        {
