...
type MyType = {
  version: number;
  type: "literal" | "+" | "*" | "Math.min" | "Math.max" | ">" | "<" | "===";
  value: number | null;
  totalLength: number;
...
const start = () => {
  ...
  const total = evaluate(packetInfo!);
...
const getType = (val: number) => {
  ...
  throw `Incorrect type: '${val}'`;
};

const evaluate = (packet: MyType): number => {
  const children = packet.children.map((x) => evaluate(x));

  switch (packet.type) {
    case "+":
      return children.reduce((previous, current) => (previous += current), 0);
    case "*":
      return children.reduce((previous, current) => (previous *= current), 1);
    case "Math.min":
